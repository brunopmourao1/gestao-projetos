...
O schema fica em `src/db/schema.ts`, espelhando `Modelo-Dados-ER.md` (tabelas `projetos`, `historico_transicoes`, `pendencias_visitas`, `checklist_itens`). Com `DATABASE_URL` configurada em `.env.local`:
...
* `src/proxy.ts` — protege o sistema com a senha única compartilhada (`APP_PASSWORD`, ver HU-21). **Atenção:** nesta versão do Next.js (16), o antigo `middleware.ts` foi renomeado pra `proxy.ts` com export `proxy` (não `middleware`) — e como o projeto usa `src/`, o arquivo precisa ficar em `src/proxy.ts`, não na raiz do repo. Ver `node_modules/next/dist/docs/01-app/03-api-reference/03-file-conventions/proxy.md`.
